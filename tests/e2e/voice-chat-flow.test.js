    ...
    await page.evaluateOnNewDocument(() => {
          ...
          getUserMedia: async (constraints) => {
            ...
            return {
              getTracks: () => [{
                ...
                kind: constraints.audio ? 'audio' : 'video'
              }],
      ...
      window.fetch = async (url, options) => {
        ...
        if (url.includes('/api/chat')) {
          console.log('Mocking chat API call');
          return {
            ok: true,
            status: 200,
            body: new ReadableStream({
              start(controller) {
                const response = 'Конечно! Вот рецепт классического украинского борща на 4 порции. Вам понадобится:';
        ...
        if (url.includes('/api/openai/tts')) {
          console.log('Mocking TTS API call');
          return {
            ok: true,
            status: 200,
      ...
      // Mock URL APIs
      window.URL.createObjectURL = (blob) => `blob:mock-${Date.now()}`;
      window.URL.revokeObjectURL = (url) => console.log('Revoking URL:', url);
    });
