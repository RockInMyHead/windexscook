    ...
    try {
      const messages = isChatMode 
        ? [
            {
              role: "system",
              content: "Ты - дружелюбный AI повар-консультант. Отвечай на русском языке простым и понятным текстом. НЕ используй JSON формат. Отвечай как живой человек-повар. ВАЖНО: Все цифры пиши словами (например: 'двадцать минут', 'триста грамм', 'пять штук'), а не цифрами (2, 300, 5)."
            },
            {
              role: "user",
              content: prompt
