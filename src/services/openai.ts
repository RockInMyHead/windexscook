    ...
    try {
      ...
      response = await fetch(requestUrl, {
        ...
        body: JSON.stringify({
          model,
          messages,
          temperature: 0.7, // Немного уменьшили для более быстрых ответов
          max_tokens: 1000, // Уменьшили с 2000 до 1000 для скорости
        }),
      });
    } catch (networkError) {
...
`;
  ...
  static async chatWithChef(
    ...
    const prompt = `Ты - профессиональный шеф-повар с многолетним опытом. Давай полезные советы по кулинарии.

Для рецептов: давай четкие пошаговые инструкции с ингредиентами, временем и техниками приготовления.

Помни контекст разговора. Отвечай кратко и по делу.

Если вопрос не про еду - направь разговор на кулинарию.

Отвечай на русском языке в женском роде. Цифры пиши словами (двадцать минут, триста грамм).`;
    ...
    try {
      ...
      // Добавляем историю сообщений (меньше в быстром режиме)
      if (messageHistory && messageHistory.length > 0) {
        const historyLimit = fastMode ? 3 : 6; // В быстром режиме только 3 последних сообщения
