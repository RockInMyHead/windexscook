...
export class EmailService {
   ...
   */
  static async sendPremiumConfirmation(email: string, userName: string): Promise<boolean> {
    try {
   ...
   */
  private static getPasswordResetTemplate(resetUrl: string, email: string): string {
    return `
      <!DOCTYPE html>
      <html lang="ru">
      <head>
   ...
   */
  private static getPasswordResetText(resetUrl: string, email: string): string {
    return `
...
© 2025 WindexsCook. Все права защищены.
   ...
   */
  private static getWelcomeText(userName: string): string {
    return `
...
Поддержка: support@windexscook.ru
   ...
   */
  private static getPremiumConfirmationTemplate(userName: string): string {
    return `
      <!DOCTYPE html>
      <html lang="ru">
      <head>
   ...
   */
  private static getPremiumConfirmationText(userName: string): string {
    return `
